#pragma once

#include "glinc.h"
#include <glm/glm.hpp>



class Game {
    public:
    Game();

    void setup();
    void gameLoop();
    void processInput(GLFWwindow* window);

    private:
    GLFWwindow* mWindow;
    unsigned int mShader;

    const glm::vec3 mBackgroundColor = glm::vec3(0.2f, 0.4f, 0.6f);
    const glm::uvec2 mWinSize = {1200,800};
    const glm::uvec2 mWorldSize = {500,500};
    const glm::uvec2 mViewSizeArg = {1000,1000};
    const char* mTitle = "Pixel World 3";

    //if mWorldSize[i] is smaller than mViewSizeArg[i], mViewSizeArg[i] is unnecessary since it's too big
    const glm::uvec2 mViewSize = {glm::min(mWorldSize.x, mViewSizeArg.x), glm::min(mWorldSize.y, mViewSizeArg.y)};
};